...
- [Create Segment API](https://staging.docs.dev.clevertap.net/docs/create-segment-api)
- [Update Segment API](https://staging.docs.dev.clevertap.net/docs/update-segment-api)
...
In CleverTap, all segments created via API are categorized as _API Segments_, ensuring they are distinct from manually created segments.

### View API Segments

After creating a segment with APIs, you can view it from the _Segment_ list page.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/91122a3844319d1f684530de3bd95a162922679b995c65f4bbbc3b153314130e-API_Segments.png",
        "",
        "View API Segment on Segment List Page"
      ],
      "align": "center",
      "border": true,
      "caption": "View API Segment on Segment List Page"
    }
  ]
}
[/block]
...
- **Online Campaigns**: When a user is added or removed from the segment created via API, CleverTap triggers a system event called _Segment Membership Change_. This event allows customers to use API segments in real-time engagement workflows, ensuring timely communication through online channels. This event includes the following properties:
  ...
  - **Customer_Segment_ID**: A reference identifier the customer provides for external tracking. For example, SEG12345.
...
3. Click the **Who** dropdown. The _Target Segment_ section opens.
4. Click **Add Rule** and then select _Segments_ from the segmentation rules pop-up.
5. Select the _API Segments_ from the available list of segments.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/86a555ff4213f6001ea7c63848955ceab609d9ac9588ddddb6191dd74ea146c2-Targeting_API_Segment.png",
        "",
        "Targetting an API Segment in a Campaign/Journey"
      ],
      "align": "center",
      "border": true,
      "caption": "Targetting an API Segment in a Campaign/Journey"
    }
  ]
}
[/block]
