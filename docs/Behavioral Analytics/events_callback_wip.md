...
[block:parameters]
{
  "data": {
    ...
    "1-0": "Identity Reset",
    "1-1": "This *Debug* event is raised when a profile is demerged (after demerged, a new profile for every device is created and identities are dropped) either from dashboard (click on the *Profile* page to reset identities) or through the *Demerge Profile API*.",
