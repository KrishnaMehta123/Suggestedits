...
* [Database Details](doc:snowflake-integration#database-details): Specify the Snowflake Account Name, Warehouse, Schema, and Database to which CleverTap should export data.
* [User Details](doc:snowflake-integration#user-details): Enter the credentials and roles for the Snowflake user authorized to access the target schema and tables.
...
3. Under *Export Details*, select the *Snowflake connection name* from the dropdown and add the following details:  
   ...
   * **DATA TYPE & IDENTIFIER PRIORITY**: Select the events from the available options to export. For more information, refer to [Export Details](doc:snowflake-export#export-details).
     * **Fallback Priority order for identities**: Select priority for user identities when exporting data. User Identifiers will be exported based on the selected priority. For more information, refer to the [Prioritize User Identity for Exports](doc:snowflake-export#prioritize-user-identity-for-exports) section. 
