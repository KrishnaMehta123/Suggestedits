---
title: Map Data for Snowflake Import
excerpt: >-
  Learn how to map user profile fields and event attributes from your Snowflake
  database to CleverTap for accurate identity resolution and seamless data
  ingestion.
deprecated: false
hidden: true
metadata:
...
After [configuring import](doc:create-import), this step lets you map your Snowflake columns to CleverTap’s user profiles or events. This ensures each row of data updates the correct fields in CleverTap. 

- [**User Profile Data**](doc:select-and-map-data#user-profile-data): Map user-specific fields such as `email`, `first_name`, `last_name`, or any other attribute from the Snowflake database, to the user profile fields in CleverTap.
- [**Event Properties**](doc:select-and-map-data#event-data): Map event-specific columns such as `add_to_cart`, `product_viewed`, `purchased` to CleverTap event properties.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/d1d1ea0c76ba66d3c3b4cb6ef1ad073b88b9324056fc8cbd9f4db557c837bc38-User_Profile_Data.png",
        "",
        "User Profile Data"
      ],
      "align": "center",
      "border": true,
      "caption": "User Profile Data"
    }
  ]
}
[/block]
...
1. Map identity to specify how incoming user records from Snowflake are matched to user profiles in CleverTap.
2. Select a timestamp column to fetch only newly added or updated records.
3. Map Snowflake columns to CleverTap user properties.

(@Krishna: Add cross-references to the step.)

## Map Identity

This step specifies how incoming user records from Snowflake are matched to user profiles in CleverTap, using either a unique identity field (such as email, phone number, or user ID) or the system-generated CleverTap ID.
...
## Map User Profile Properties from Snowflake
...
| **Date**      | Timestamps such as signup or login date      | `2024-03-18`, `18/03/2024`, `03/18/2024`             | Must match selected date format. For more information, refer to [Supported Date Format](<>). |
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/80338755cccb4898f5cf092170a96ffcde306da1e9a0f6e54a2e17568a81a04b-Event_Data.png",
        "",
        "Event Data"
      ],
      "align": "center",
...
1. Map identity to associate event data with user profiles in CleverTap.
2. Select a timestamp column to fetch only newly added or updated rows.
3. Define when each event occurred using a created-on timestamp.
4. Choose how events are named (from a column or a static value).
5. Map Snowflake columns to CleverTap event properties.

## Map Identity

This step specifies how incoming event records from Snowflake are matched to user profiles in CleverTap using either a unique identity field (e.g., email, phone, or user ID) or the system-generated CleverTap ID.
...
For more information, refer to [Supported Date Format](https://staging.docs.user.clevertap.net/docs/data-mapping-snowflake-import#:~:text=Supported%20Date%20Format).
...
## Map Event Properties from Snowflake
...
| **Date**      | Timestamps such as event or checkout time     | `2024-03-18`, `03/18/2024`  | Must match supported format. For more information, refer to [Supported Date Format](<>). |
...
| **Snowflake Database Nickname** | Displays the nickname of your Snowflake database being used for import.                            |
| **CleverTap Property**          | Shows the corresponding CleverTap property (user profile or event property) mapped from Snowflake. |
...
- **Successful Validation**: Indicates that the mapping and sample records meet CleverTap’s data structure and formatting requirements.
  ...
  - Data types are properly aligned between Snowflake and CleverTap.
...
When configuring Snowflake import mapping in CleverTap, keep the following best practices and tips in mind to make the most of these features:
...
- **Consistent Formats**: Keep date and time formats uniform in Snowflake. If you choose Date type, ensure the column’s format (YYYY-MM-DD, etc.) matches what you set in CleverTap. Inconsistent formats will cause parsing errors.
...
- **Security and Access**: Use a dedicated Snowflake user with minimal permissions for the integration. Grant only read access to the tables/columns you need.
...
- **Segmentation of Data**: Consider filtering your Snowflake data to exclude obsolete or inactive users/events before import. This keeps your CleverTap instance clean and focused on relevant customers.

By understanding these options and best practices, you can ensure your Snowflake-to-CleverTap imports run smoothly and keep your customer data accurate and up-to-date.
