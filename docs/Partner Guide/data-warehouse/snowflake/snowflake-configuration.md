...
<summary>Expand to learn how to interpret CleverTap's release tags.</summary>
<hr />
...
* Schema (optional): If not specified, the public schema is used by default.
...
2. Enter the following details: *Database*, *User*, and *Warehouse* and *schema (optional)*.
...
* For new users who need to provision Snowflake resources from scratch: [Create a new Database, Warehouse, User, and Role](doc:snowflake-integration#creating-a-new-database-warehouse-user-and-role)
* For users who already have configured resources in Snowflake: [Use existing Snowflake Credentials](doc:snowflake-integration#using-existing-snowflake-credentials) 
...
1. [Create a Database](doc:snowflake-integration#create-database)
2. [Create a Warehouse](doc:snowflake-integration#create-warehouse)
3. [Create a User](doc:snowflake-integration#create-user)
4. [Create a Schema ](doc:snowflake-configuration#create-schema)
5. [Create a Role](doc:snowflake-integration#create-role)
...
1. Create a role for CleverTap integration:  
   ...
   ```
2. Grant the required privileges to enable CleverTap to read from and write to the relevant schemas:\
   CleverTap requires write access to the schema where export tables are created. If you are using the `PUBLIC` schema, no additional permissions are needed. However, if you are using a custom schema, replace `<schema_name>` with the actual schema name and grant the following privileges: 
...
After creating the database, you must create a schema to organize CleverTap-related objects. To do so, perform the following steps:
...
* [Obtain Your Snowflake Account Identifier](doc:snowflake-integration#obtain-your-snowflake-account-identifier)
* [Find Existing Database](doc:snowflake-integration#find-existing-database)
* [Find Existing Warehouse](doc:snowflake-integration#find-existing-warehouse)
* [Find and Assign Existing Role](doc:snowflake-integration#find-and-assign-existing-role)
...
2. Click ![](https://files.readme.io/ad0880e14157a8d9ab232c369d6fd326e4dd0d0e41496d08e55da5dea9df94c2-Screenshot_2025-03-03_at_11.28.40_AM_1.png) and select *Manage URL* from the dropdown and copy the **Current URL**, which is typically in the following format:
...
* [Database Details](doc:snowflake-integration#database-details)
* [User Details](doc:snowflake-integration#user-details)

## Database Details

Enter the *Database Details* in the integration setup form. To create or retrieve details from your Snowflake account, refer to [create a new Database, Warehouse, User, Role, and Schema](doc:snowflake-integration#creating-a-new-database-warehouse-user-and-role)  or [use existing Snowflake credentials](doc:snowflake-integration#using-existing-snowflake-credentials).

<Image alt="Database Details" align="center" border={true} src="https://files.readme.io/ceac4ae27983f57f2b62c58ac1451af192191eeabc9c8964f501363c833d679e-snowflake_databse_details.png" />  Database Details
...
| Field              | Description                                                                                                         |
| ------------------ | ------------------------------------------------------------------------------------------------------------------- |
| Connection name    | A unique name that you will use further to identify your configuration while setting up imports or exports.         |
...
| Schema             | The specific schema within the database that contains the data.                                                     |
...
* [Key Pair Authentication](doc:snowflake-integration#key-pair-authentication) [Recommended]
* [Password Authentication](doc:snowflake-integration#password-authentication)
...
<Image alt="User Credentials - Key" align="center" border={true} src="https://files.readme.io/c8b490bb84fc69169cb1dd26c268ba5288416467daa9b1b6f252cec89b0e32e1-snowflake_user_credentials_key.png" />  User Credentials - Key
...
6. After saving the Snowflake Connection, *[create Import](doc:snowflake-import)* from the *Import Connections* dashboard or *[Create Export](doc:snowflake-export)* from the *Export Connections*.
...
<Image alt="User Credentials - Password" align="center" border={true} src="https://files.readme.io/6bce82ac0999297f38bdb32d599e99de9748da8d01b395290aa1c36fcdf49a62-snowflake_user_credentials_-_password.png" />  User Credentials - Password
