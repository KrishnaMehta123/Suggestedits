...
<summary>Expand for quick setup if your BigQuery environment is already configured.<hr></summary>
...
Once you have these values, you can now proceed to add them in CleverTap (see _Set Up CleverTap Dashboard for Integration_).
...
1. In the Google Cloud Console, navigate to _IAM & Admin > Service Accounts_.
...
1. Go to _IAM & Admin > IAM_. Go to the _Service account_.
...
1. In _IAM & Admin > Service Accounts_, select the service account.
2. Open the **Keys** tab.
...
```sql
CREATE SCHEMA IF NOT EXISTS `your-project-id.clevertap_dataset`;
```
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/90a4cea28203917d02cabc0c6ea3483bd5344657ba5d867f4446f0b1b1d5d9ac-image_74.png",
        "",
        "Integrate BigQuery with CleverTap"
      ],
      "align": "center",
      "border": true,
      "caption": "Integrate BigQuery with CleverTap"
    }
  ]
}
[/block]
