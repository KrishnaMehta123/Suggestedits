...
title: AWS S3
excerpt: >-
  Learn how to set up AWS S3 dashboard to export data from CleverTap dashboard
  to AWS S3 Bucket.
deprecated: false
hidden: true
metadata:
...
1. [Create an AWS S3 Bucket](doc:configure-aws-s3#create-an-aws-s3-bucket).
2. Configure Buckets details on the CleverTap Dashboard using one of the following methods:
   - [Using Access Key](doc:configure-aws-s3#using-access-key)
   - [Using IAM Policy](doc:configure-aws-s3#using-iam-policy)
...
3. Click **Edit** under the _Bucket policy_ section and enter the policy that you copied in Step _4_ of [Copy IAM Policy from the CleverTap dashboard](doc:configure-aws-s3#copy-iam-policy-from-the-clevertap-dashboard):
...
5. Go back to the same window opened in _Step 4_ of [Copy IAM Policy From CleverTap Dashboard](doc:configure-aws-s3#copy-iam-policy-from-the-clevertap-dashboard) and click **Save Credentials**.
