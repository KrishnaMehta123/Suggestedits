...
- Once the messaging endpoints and templates have been saved and tested successfully, Partner can [create a WhatsApp campaign](https://docs.clevertap.com/docs/create-message-whatsapp).  The campaigns must target a large user base to scale-test the integration.
...
- Respond to users from [agent chat](https://docs.clevertap.com/docs/conversations#role-based-access-agent-role) with different combinations ( text, media, location, documents, etc) and check whether the messages are being delivered to end users correctly.
