...
2. Toggle on the *Event callback* option on the Sendbird Dashboard.\
   This helps integrate the delivery receipt and read receipt events of the notification with CleverTap's [custom events](doc:events#custom-events). When an event takes place, the Sendbird server uses CleverTap's [upload events API](doc:upload-events-api) to raise an event on the CleverTap.\
