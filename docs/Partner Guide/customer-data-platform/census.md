...
4. (Optional) Click **Test**  to test the connection and verify the credentials you have entered.
...
1. **Add a Data Source**:
   1. Open the _Sources_ section from the Census dashboard and click **+ New Source**. 
...
5. _Select a  Sync Behavior_. The sync behavior determines how changes are handled between the source and CleverTap. If you select **Send**, an event is raised on CleverTap every time new records are detected in the source system.
...
7. Under _Set Up CleverTap Field Mappings_, select whether you want to sync specific properties or all properties.  
   When you add a mapping, Census automatically matches standard properties in CleverTap. You can also create new properties in CleverTap if needed. It is mandatory to sync **Event Name** (Action) and **User Identity** (Customer ID).
   ...
   - ** Map Columns**: Map the source columns (for example, `user_email`) to corresponding fields in CleverTap. This ensures the correct data is sent to the right fields.
...
Incorrect mapping can lead to incorrect data being updated in CleverTap. To resolve the issue and re-sync, you can modify the field mapping in Census.
