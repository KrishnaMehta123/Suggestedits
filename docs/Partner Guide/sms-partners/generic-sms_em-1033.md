---
title: Generic SMS_EM-1033
excerpt: >-
  Learn how to integrate any SMS provider with CleverTap via HTTP integration.
  Configure authentication, message payloads, callbacks, and track delivery
  reports.
deprecated: false
hidden: false
link:
...
<Image title="1 Add Provider dashbboard.png" align="center" className="border" border={true} src="https://files.readme.io/6c72583-1_Add_Provider_dashbboard.png" />
...
<Image title="2 Under Authentication.png" align="center" className="border" border={true} src="https://files.readme.io/c6435fb-2_Under_Authentication.png" />
...
<Image title="under_Parameters.png" align="center" className="border" border={true} src="https://files.readme.io/23dd8e3-under_Parameters.png" />
...
<Image title="Screenshot 2021-05-12 at 4.35.40 PM.png" alt="JSON Payload Structure for Outgoing Requests" align="center" border={true} src="https://files.readme.io/a8e7b8d-Screenshot_2021-05-12_at_4.35.40_PM.png" />
...
SMS callbacks help you track detailed reports of every SMS being sent from the CleverTap dashboard. This includes information about the following events for SMS campaigns: Delivered, Replied, and Clicked. Furthermore, it provides details of delivery-related errors.

The Callback URL is where the delivery stats for SMS campaigns are received. The SMS providers must add this URL to their SMS Provider Settings. You can find this information under the Provider *Setup* tab on the CleverTap dashboard.

<Image title="Callback URL.png" alt="Callback URL on CleverTap Dashboard" align="center" border={true} src="https://files.readme.io/64207ba-Callback_URL.png" />

> 📘 Key Points to Remember
>
> The following are the key points to remember when using CleverTap's Generic SMS Callback feature:
>
> * The SMS callback feature is available only for the Generic SMS Integration currently.
> * If you already have a provider saved, you must set up a new one with the same configuration details along with additional information for SMS callbacks explained in the [Configure Callback Events](#configure-callback-events) section.
> * Do not set up the Generic Callback feature if you already use CleverTap's Click Tracking feature to avoid duplication of events.
...
<Image alt="SMS Callback Working" align="center" border={true} src="https://files.readme.io/3d8f3d1-SMS_Callback_Configuration.png" />
  ...
  "msgs": [
    {
      ...
      "mid": "9999999999.1200000000.1654002362.20220531.0.wzrk_default.-1.",
      "to": "+919999999999",
      "body": "Hi User test notification body message"
    },
    {
      "tid": "1107160016156755389",
      "mid": "8888888888.1200000000.1654002362.20220531.0.wzrk_default.-1.",
      "to": "+918888888888",
      "body": "Hi User test notification body message"
    }
  ],
...
<Table align={["left","left","left"]}>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Denotes Template ID.<br />
        An optional replacement value used to signify the key where the TemplateID of the message is added to the payload.<br />
        Every configuration must have $$TemplateID or $$Body key as a mandatory replacement.<br />
        This variable is optional and only applicable for campaigns targeted to Indian mobile numbers.
      </td>
      <td>
        Replacement - $$TemplateID<br /><br />
    ...
    <tr>
      ...
      <td>
        Denotes Message ID.<br />
        Metadata that CleverTap sends to the provider and receives in the incoming payload if the callbacks are configured.<br />
...
| :-------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------- |
| event           | Denotes the event name.                                                                                                                                                                         | failed/failed_dnd/replied/clicked                          |
...
| incomingMessage | User-originated message text. It is the reply string in case of a replied event                                                                                                                 | STOP, START                                                 |
| url             | URL in case of the click event                                                                                                                                                                  | www.CleverTap.com                                           |
| shorturl        | Short URL in case the outbound URL has been shortened                                                                                                                                           | ct9.io/12345                                                |

> 📘 Note
>
> If the payload includes the keys that are not defined in the acceptable sample payload for each event, CleverTap does not throw any errors. Instead, CleverTap disregards such keys.
...
```json
{
  ...
  "data": [{
    ...
    "ts": 1435322805,
    "meta": "8888888888.1200000000.165263328.20220606.0.wzrk_default.1.",
    "description": "Delivered to handset"
  }]
}
```
...
<Table align={["left","left","left"]}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        The message dropped due to one of the following reasons:<br />
        <ul>
          <li>SMS provider's rule</li>
          <li>Invalid numbers in the uploaded base, or</li>
          <li>The client has requested to drop.</li>
        </ul>
      </td>
    </tr>
    <tr>
...
## Delivery Callback Payload Mapping
...
<Table align={["left","left","left"]}>
  ...
  "data": [{
    ...
    "description": "Delivered to handset"
      ...
      <td>
        Maps to Notification Delivered event.
      </td>
    </tr>
    <tr>
      <td>
  ...
  "data": [{
    ...
    "shorturl": "ct9.io/12345"
      ...
      <td>
        <ul>
          <li>Event property <code>url</code> in the <code>IncomingText</code> maps to Notification Clicked.</li>
          <li>Event property <code>shorturl</code> in the <code>IncomingText</code> maps to the event property <code>wzrk_c2a_short_url</code> of the Notification Clicked event.</li>
        </ul>
      </td>
    </tr>
  </tbody>
...
>
> CleverTap trims the Notification replied text containing more than 512 characters.
...
| :---------- | :--------------------------------------------------------------------------------------- |
| 200         | -                                                                                        |
| 400         | Bad request. This status code is sent when some of the mandatory parameters are missing. |
| 401         | Unauthorized                                                                             |
| 500         | Server error                                                                             |
...
{
  "status": "error",
  "message": "Bad request: Missing required parameter 'meta'",
  "timestamp": 1654512921,
  "code": 400
}
```
...
To track and manage SMS replies in CleverTap, configure your SMS provider to forward incoming messages to CleverTap. This setup ensures that user responses are captured and processed efficiently.

1. Navigate to *Settings > Channels > SMS* and select the **Provider**. Locate **Incoming Message Callback URL**. The Incoming Message Callback URL on the CleverTap dashboard is displayed as a non-editable field after adding a new provider. Copy this URL and enter it in your SMS provider's callback URL field. Forward the incoming messages in the formats explained below to this callback URL.

Once configured, SMS replies will be processed within CleverTap, enabling better engagement tracking and response management.

### Payload Format

The incoming SMS reply must be sent in the following JSON format:
...
```json
{
  ...
  "data": [{
    "ts": 1654512921,
    "meta": "9876543210.1200000000.165451292.20220606.0.wzrk_default.1.",
    "toPhone": "1234",
    "incomingMessage": "Hello, I'm interested in your product"
  }]
}
```
...
* *Stats* tab of the Campaigns. For more information, refer to [SMS Setup](https://docs.clevertap.com/docs/sms-stats).

<Image alt="SMS Campaign Stats" align="center" width="60%" border={true} src="https://files.readme.io/470db6b-SMS_Camapign_Stats_page.png" />
