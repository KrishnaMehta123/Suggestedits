...
The Callback URL is where the delivery stats for SMS campaigns are received. The SMS providers must add this URL to their SMS Provider Settings. You can find this information under the Provider _Setup_ tab on the CleverTap dashboard.
...
> - If you already have a provider saved, you must set up a new one with the same configuration details along with additional information for SMS callbacks explained in the [Configure Callback Events](doc:generic-sms_em-1033#configure-callback-events) section.
> - Do not set up the Generic Callback fetaure if you already use CleverTap’s Click Tracking feature to avoid duplication of events.
...
[block:parameters]
{
  "data": {
    ...
    "1-0": "mid",
    "1-1": "Denotes Message ID.  \nMetadata that CleverTap sends to the provider and receives in the incoming payload if the callbacks are configured.  \nCleverTap dynamically replaces the mid with the required metadata, which is used to  consume callbackevents and display them on the CleverTap dashboard",
...
```json
[{
		"event": "failed_dnd", //in the case of errors in delivery when customer enables DND
		"data": [{
				"ts": <10-digit Epoch Timstamp in seconds>,
				"meta": "<replaced by value received in $$MessageID in the outgoing message payload>",
				"code": "<ErrorCode>",
...
> 📘 Note
> 
> If the payload includes the keys that are not defined in the acceptable sample payload for each event, CleverTap does not through any errors. Instead, CleverTap disregards such keys
...
[block:parameters]
{
  "data": {
    ...
    "0-1": "{  \n\t\"event\": \"delivered\",  \n\t\"data\": [{  \n\t\t\"ts\": 1435322805,  \n\t\t\"meta\": \"9999999999.1200000000.165263328.20220606.0.wzrk_default.1.\",  \n\t\t\"description\": \"Delivered to handset\"  \n\t}]  \n}",
    "0-2": "Maps to Notification Delivered event.",
    "1-0": "Notification Clicked",
    "1-1": "{  \n\t\"event\": \"clicked\",  \n\t\"data\": [{  \n\t\t\"ts\": 1435322805,  \n\t\t\"meta\": \"9999999999.1200000000.165263328.20220606.0.wzrk_default.1.\",  \n\t\t\"url\": \"www.CleverTap.com\",  \n                \"shorturl\": \"ct9.io/12345\"  \n\t}]  \n}",
    "1-2": "<li> Event property `url` in the `IncomingText` maps to Notification Clicked. </li> <li> Event property \\``shorturl` in the `IncomingText` maps to the event property `wzrk_c2a_short_url` of the Notification Clicked event."
  },
  "cols": 3,
  "rows": 2,
  "align": [
...
1. Navigate to _Settings> Channels> SMS_ and select the **Provider**. Locate **Incoming Message Callback URL**.  The Incoming Message Callback URL on the CleverTap dashboard is displayed as a non-editable field after adding a new provider. Copy this URL and enter it in your SMS provider's callback URL field.  Forward the cincoming messages in the formats explained below to this callback URL.
...
- _Stats_ tab of the Campaigns. For more information, refer to [SMS Setup](https://docs.clevertap.com/docs/sms-stats).
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/470db6b-SMS_Camapign_Stats_page.png",
        null,
        "SMS Campaign Stats"
      ],
      "align": "center",
