...
CleverTap follows the Cross-Origin Resource Sharing (CORS) standard. The CORS standard uses new HTTP headers so that browsers can request remote URLs. However, these requests are fulfilled only if they are permitted from the client-side. The API requests from the frontend of your website do not pass this standard. Therefore, all the API calls must be made from the server-side. {@harsh, I have revamped the whole description. please check for gaps}
