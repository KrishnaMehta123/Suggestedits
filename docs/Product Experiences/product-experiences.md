...
4. Select the Segments and Keys for your Product Config group. The Keys are the keys that you have assigned in your app for each element or feature and the Segments are the group of users that will receive this feature. In our example, we select the "iPhone users in New York"  and "iPhone users in Miami" as Segments and "Dark_Key" as the Product Config Key. For more information on creating keys, see [Keys](https://docs.clevertap.com/docs/product-config#section-create-keys)
[block:callout]
{
  "type": "info",
