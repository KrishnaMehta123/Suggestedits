...
Create a campaign to deliver your Webhook. To create a new campaign, perform the following:

1. On the CleverTap dashboard, select *Campaigns*.
2. Click **+ Campaign**.
3. From the *Messaging Channels* list, select **Webhook**. 
...
<Image title="Webhook_editor_main.png" alt={1084} className="border" width="80%" border={true} src="https://files.readme.io/edea828-Webhook_editor_main.png" />
...
* Qualification criteria: Deliver the notification by [Past Behavior](https://docs.clevertap.com/docs/push-campaign#section-Webhook-based-on-past-behavior-pbs), [Custom List](doc:custom-list-segments), [events](https://docs.clevertap.com/docs/push-campaign#section-deliver-action-based-push-notifications), or API. 
...
![919](https://files.readme.io/7d83483-Push_notification_set_goal_track_conversion.png "Push_notification_set_goal_track_conversion.png")

### Select Webhook

Select the available Webhook from the list.

## Define the Who

You must indicate the target audience for your Webhook campaign. You can target your Webhook campaign to a new user segment by clicking on the **Target segment** section. Here, you can create a new ad-hoc segment or use a previously saved user segment from the *segment* list. 
...
If you want to create an ad-hoc segment, you can select a type of segment on which to base your Webhook campaign. You can create the target on the basis of past user behavior and/or user properties, or live (ongoing) user behavior (the latter is useful to send out real-time, triggered Webhook campaigns).
...
You can trigger a Webhook message based on an action. Users receive Webhook notifications when they perform an action in the app instead of waiting for the next app launch. This makes notifications more contextual and increases conversion. The instant Webhook message is not triggered for campaigns with delays or when the app inbox is coupled with a Webhook message.

### Deliver Webhook Notifications based on Past Behavior (PBS)

You can also target users their past user behavior. For past behavior campaigns, you also have an option to calculate estimated reach to view how many users qualify for the campaign criteria and how many are reachable via Webhook as of now.

### Filter by User Properties

Using the *With user properties* filter in the *Who* section, you can segment your campaign to only reach users who meet specific criteria. 

For example, you can send a Webhook notification to English-speaking female users who live in the United States. 

<Image title="campaigns_user_properties.png" alt={516} className="border" border={true} src="https://files.readme.io/dbf1c37-campaigns_user_properties.png" />
...
![1048](https://files.readme.io/6dfb26c-Webhook_what_without_options.png "Webhook_what_without_options.png")
...
![1152](https://files.readme.io/6a7e4ff-Webhook_editor_what.png "Webhook_editor_what.png")

You can either use *Profile Variables* or *Custom* body.  You can also add personalization with the @ symbol. You have the option to send/receive an email, identity, objectId (CleverTap ID), profileData (custom profile variables), and push\_token for each user.  \<\<@vishal, dk, need a sample screen here>>
...
You can also choose a custom payload. \<\<@vishal, dk, need a sample screen here>>
...
\<\<@vishal, dk, need a sample screen here>>
...
Global campaign limits are limits you can apply to determine how many Webhook notifications each app user receives per day. If you want to override these settings for an important campaign, you can click on the *Don’t apply global campaign limits to this campaign* checkbox.

You can also specify *Do Not Disturb* (DND) hours during which notifications from this Webhook campaign are prevented from going out, either by discarding them or delaying delivery to after DND hours complete, such as 9 PM to 9 AM.
...
<Image title="Webhook_delivery_preference.png" alt={801} className="border" border={true} src="https://files.readme.io/d8ce9a1-Webhook_delivery_preference.png" />
