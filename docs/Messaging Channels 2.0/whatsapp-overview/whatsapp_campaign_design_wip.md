---
title: WhatsApp_campaign_design_WIP
excerpt: ''
deprecated: false
hidden: true
metadata:
...
Select **WhatsApp** from the *Messaging channels* list. 
[block:image]
{
  "images": [
    {
...
It can be based on a specific time or event. 
...
Click the *Add a new service provider* link to add more service providers. 

### Goal

Create a goal for the campaign. 
Enable Conversion Tracking and select a conversion event. 

## Define the Who

You must indicate the target audience for your *WhatsApp* campaign. You can target your campaign to a new user segment or use a previously saved user segment from the *Custom list*. You can also use an *External user list*.
[block:image]
{
  "images": [
    {
...
[/block]
Set a control group to compare your campaign results. 
CleverTap allows you to cap the target audience to avoid any unnecessary spending.
...
From the delivery preferences section, select **Do Not Disturb (DND)**, to avoid sending messages during the DND period specified by the user. 
