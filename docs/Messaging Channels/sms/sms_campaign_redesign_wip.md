---
title: SMS_Campaign_redesign_WIP
excerpt: ''
deprecated: false
hidden: true
metadata:
...
3. Select *SMS* from the  *Messaging channels* list. 
...
Set up the campaign from the *Setup* section. 

1. Select the delivery type. 
2. Select the SMS provider or provider group.
3. Set a goal with conversion tracking. 
...
\<\<@vishal how are the ad-hoc segments handled in the new design?>>
...
For example, we can create a live, *Inaction within time* campaign that targets users as soon as they add a product to the cart but do not finish transacting within five minutes, the golden window within which most users transact on our iOS and Android app platforms.
...
To set the delivery preferences, use the following to make your selection:

Past behavior campaigns can be scheduled to run:

* Send Now: On a specific date and time.
* Schedule for later: On multiple specified dates and times.
* Set as Recurring: On a recurring basis.
