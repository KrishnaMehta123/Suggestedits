...
Multiple opens means that the recipient opened your email numerous times.  However, there may be instances where the email _opens_ exceeds expectation, or _opens_ for a recipient are registered multiple times in quick succession. It can be due to the following reasons:
...
- Email Forwards - The forwarded email is an exact copy of the original email. Forwarded emails can also add to the _open_ count.  The open is attributed to the original recipient, even if the recipient of the forwarded email opens the email. 
