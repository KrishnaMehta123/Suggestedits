...
CleverTap's App Functions feature allows you to modify an app's functionality and appearance without requiring users to download an updated version. You can customize specific actions when users engage with the app, such as directing them to the in-app store, awarding virtual coins, or sending a discount code. You can save these actions as templates for easy use in in-app messages.

Additionally, you can discreetly activate App Functions without displaying an in-app message, such as sending a user back to a previous point in a game when they open the app or providing them with daily coin rewards. These functions seamlessly integrate into regular campaigns. For more information, refer to the app functions \{developer docs}.
