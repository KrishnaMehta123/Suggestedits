...
The recommendations feature is critical in helping marketers engage and retain their customer base as it provides the capability to create personalized content for each user based on their unique historical data. 
...
* Then, you can create a recommendation by defining certain rules that indicate which catalog you want to use, the catalog value, the event based on which recommendation is to be generated, the lookback window, and any specific filtering criteria.
...
* **Triggered Campaigns**\
    ...
    **Example**\
  Consider the example where Charged event is the event defined for the last action under recommendations, and Added to cart is the qualifying event. 
