...
- Correcting points in cases where automated transactions need manual intervention.
- Deducting points in case of fraudulent activity or errors.

The following is an example of manually crediting points for a user who reported a poor experience. For instance, a customer whose order was delivered two days late can be compensated by manually crediting 50 loyalty points to their wallet as a goodwill gesture.
...
Before submitting a manual points adjustment, ensure the following conditions are met to avoid validation errors:

- The number of points to be debited must not exceed the user's current active wallet balance.
- Only positive numeric values are allowed for the transaction value.
- All mandatory fields, such as _User identity_ or _Description_, must be filled in.
...
Any points credited through a [Promo Campaign](doc:promo-campaigns) or [Credit API](https://staging.docs.dev.clevertap.net/docs/debitcredit-wallet) with a custom activation duration are first added to the Promised Points Bucket. After the activation duration is completed, the same points are moved to the Active Points Bucket. The users cannot use points from the promised bucket for redemption.
...
- **API**: Credit or debit transactions are initiated via the [Debit/Credit API](https://staging.docs.dev.clevertap.net/docs/debitcredit-wallet) calls.
...
- **A/B Test Wallet Campaigns:** Manually experiment with different point values and promotions to optimize results.