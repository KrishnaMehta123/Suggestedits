...
[block:parameters]
{
  "data": {
    ...
    "7-2": "If Per Campaign Budget = 5,000 points, rewards stop once the campaign collectively distributes 5,000 points, even if more users qualify.  \nIf Per User Budget = 500 points, each user can only receive up to 500 points during the campaign, even if they continue qualifying.",
...
[block:parameters]
{
  "data": {
    ...
    "2-1": "Defines how many times the same user can redeem a coupon. This does not control redemption. If a user-level redemption limit is 2 and the same coupon is rewarded twice, user can redeem 4 times total.",
...
| **Max codes per user**     | Defines how many times the same user can redeem a coupon. This does not control redemption. If a user-level redemption limit is 2 and the same coupon is rewarded twice, user can redeem 4 times total. | Selecting _Limit = 1_ means each user can only be rewarded the coupon once.                           | Live Behavior Segments only                        |
| **Max codes per campaign** | Defines the total number of coupon codes that the campaign can distribute.                                                                                                                              | Selecting _Limit = 1000_ means the campaign stops distributing coupons after 1000 coupons are issued. | Live Behavior Segments                             |
...
[block:parameters]
{
  "data": {
    ...
    "13-2": "For example, refer to [JSON Payload](doc:create-promo-campaigns-copy-1#json-payload)",
