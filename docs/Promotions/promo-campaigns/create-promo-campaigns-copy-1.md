---
title: Create Promo Campaigns (New)
excerpt: >-
...
(@Bajrang: this is what I suggest. Please look into the intro for this section.)
...
When Loyalty Point is selected, the following configuration options are available: (@bajrang: options are not bulleted. Please do that. Colon inconcistency - some are bold and some are not - please chnages the ones thagt are bold. we usually dont bold the colon.))

(@Bajrang/Krishna: we shd also add a reference to the respective reward type docs if the users want to know more about each of them.)
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * \*Proportional\*\*: Points awarded per defined unit of an event property (for example, 10 credits per Rs. 100 spent).  
        * \*Random\*\*: Points assigned from a defined range; exposes Value Range + Distribution Bias controls. (@Bajrang: better explanation.)
      </td>
      ...
      <td>
        ...
        * \*Proportional:\*\* For every ₹100 spent, the user gets 10 points. (@Bajrang: how is this proportional?? Please give a better example here.)  
        * \*Random:\*\* Users earn between 10 and 50 points, with distribution bias favoring the average. (@Bajrang: this is not clear.)
      </td>
    ...
    <tr>
      ...
      <td>
        ...
        * \*Favor Max\*\*: Skews distribution toward the higher end of the range.  
        ...
        (@Bajrang: Better examples for these and when we use each of them????)
      </td>
    ...
    <tr>
      ...
      <td>
        If Per Campaign Budget = 5,000 points, rewards stop once the campaign collectively distributes 5,000 points, even if more users qualify.\
        If Per User Budget = 500 points, each user can only receive up to 500 points during the campaign, even if they continue qualifying.
      </td>
...
<Table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Defines how many times the same user can redeem a coupon. This does not control redemption. If a user-level redemption limit is 2 and the same coupon is rewarded twice, user can redeem 4 times total.
      </td>
...
<Image alt="Set Up Coupons in Rewards" align="center" border={true} src="https://files.readme.io/362ace8d3ea7a677c7c9a93f885bd75dbd8db80a5f74fc2fda32e1de4b2c66ca-image.png" />  Set Up Coupons in Rewards
## Partner Vouchers
...
| **Max codes per user**     | Defines how many times the same user can redeem a coupon. This does not control redemption. If a user-level redemption limit is 2 and the same coupon is rewarded twice, user can redeem 4 times total. | Selecting *Limit = 1* means each user can only be rewarded the coupon once.                           | Live Behavior Segments only                        |
| **Max codes per campaign** | Defines the total number of coupon codes that the campaign can distribute.                                                                                                                              | Selecting *Limit = 1000* means the campaign stops distributing coupons after 1000 coupons are issued. | Live Behavior Segments                             |
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * \*Self-managed wallet credits\*\*: Supports per-campaign and per-user limits (varies by scheme). (@Bajrang: the following shd be sub-bullets)  
