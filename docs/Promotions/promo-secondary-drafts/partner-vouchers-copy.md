...
When a user receives a voucher, the Promo engine triggers an event called _CT Partner Voucher Rewarded_ with its event metadata. Based on this event, you can create a live-action engagement campaign, and the event metadata can be used to send a personalized message using Liquid tags to the user upon receiving the reward.
...
View and manage all your voucher lists in one place. The _Voucher Lists_ page provides key details about each list, helping you track their status, usage, and associated campaigns. To access the _Voucher Lists_ page:
...
| **List Tag**               | A unique identifier representing the voucher codes within the list. List Tag is used along with Provider Name to generate a unique List ID.                                                   |
| **Updated On**             | The date and time of the most recent modification.                                                                                                                                            |
| **Status**                 | <p>Indicates the current state of the voucher list. The following are the statuses:</p><ul><li>Active (Green)</li><li>Expired (Red)</li><li>Paused (Orange)</li><li>Archived (Gray)</li></ul> |
...
To view the upload history of a voucher file, click the **List ID** of the desired voucher list, then click the _Uploads_ tab.
...
| Column Name                | Description                                                                                                                                                                                                                                                                                                                                                 |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Timestamp**              | The date and time when the voucher file was uploaded.                                                                                                                                                                                                                                                                                                       |
| **Status**                 | <p>Indicates the current state of the upload process. The possible statuses are:</p><ul><li>**Completed**: The upload was successful, and all valid voucher codes have been processed.</li><li>**In Progress**: The system is still processing the uploaded file.</li><li>**Failed**: The upload encountered errors, and no codes were processed.</li></ul> |
| **Uploaded By**            | Displays the email ID of the user who uploaded the voucher file.                                                                                                                                                                                                                                                                                            |
| **File Name**              | Name of the uploaded file.                                                                                                                                                                                                                                                                                                                                  |
| **Codes (Uploaded/Total)** | Number of voucher codes successfully uploaded compared to the total codes in the file.                                                                                                                                                                                                                                                                      |
| **Errors**                 | Displays any issues encountered during upload, such as duplicate codes or an invalid file format. It also indicates the total number of errors found in the uploaded file.                                                                                                                                                                                  |
...
> 📘 Upload Codes
> 
> If your voucher list is running low on codes, you can add more voucher codes without creating a new list. The _Upload Codes_ option let's you do this which ensures uninterrupted voucher distribution in your campaigns.
> 
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/200d13a916693d097cdb3fd7a5667fbdc20fd3a39dcf3f2ea25c957b3e082209-Create_Voucher_List.png",
        null,
        "Voucher List Setup"
      ],
      "align": "center",
      "border": true,
      "caption": "Voucher List Setup"
    }
  ]
}
[/block]
...
the TheCleverTap system enforces strict duplicate checks while uploading voucher codes to ensure data integrity. The duplicate check logic is based on the combination of _List Tag_ and _Partner_ across all _Active_ voucher lists (lists that are not archived).
...
After a voucher is assigned to a user, CleverTap triggers the _Partner Voucher Rewarded_ event, which includes key properties such as:
...
You can drive engagement by sending a communication campaign using the _Partner Voucher Rewarded_ event. These properties can be leveraged in Liquid Tags to personalized messages, enabling dynamic content in push notifications, emails, or SMS.
...
4. Select the _Partner Voucher Rewarded_ event and the required _Event Property_.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/caf5147da03def348122c8efcd3d085d43af1f932de05043eca578f1336702d9-image.png",
        null,
        "Target Users with a Campaign/Journey"
      ],
      "align": "center",
      "border": true,
      "caption": "Target Users with a Campaign/Journey"
    }
  ]
}
[/block]
...
- **Error Uploading Codes**:
  - Ensure the file format is _.CSV_.
...
The _Codes Issued_ column provides real-time data on the number of voucher codes distributed to users.
