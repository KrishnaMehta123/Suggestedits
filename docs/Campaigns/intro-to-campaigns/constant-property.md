...
As a growth marketer for an E-Commerce app, you want to segment out an audience that has Viewed a product in the past but not bought that exact product. A straightforward segmentation would be to qualify people based on each product - Users who did Product Viewed Event where Product Name = Red Shoes in the last 30 days and Users who did not do Charged Event where Product Name = Red Shoes in the last 30 days. This would need to be repeated for all the items the E-Commerce app has available.
...
If User A has Viewed "Jenny red shoes" and not Purchased that exact product, then they would get an engagement as -\
Hello A\
You have added **Jenny's red shoes** to your cart. How about a discount code, Cashback20, to help you save more?

If User B has Viewed "Wonderland yellow jacket" and not Purchased that exact product, then would get an engagement as -\
Hello B\
You have added a **Wonderland yellow jacket** to your cart. How about a discount code, Cashback20, to help you save more?

## Example #2 - Summary engagement

As a growth marketer for the E-Commerce app, you may want to send a summary of Weekly Expenses and Savings on the User's transactions on the application. This can nudge the users to continue seeing value in purchasing from your E-Commerce platform.

The following are some sample engagement messages where John spent $100 over the last week and saved $20 using various coupons. Mary spent $120 the previous week and saved $40 using multiple coupons. You can send out a notification that says something like the following:
...
Holding an event property constant across [Past Behavior Segments](doc:create-past-behavior-segments) achieves the desired message for users.
...
<Image title="Select_Channel_Campaign.jpg" alt={2868} align="center" border={true} src="https://files.readme.io/010e0ef39bab1457c80bdbc7e80479d07c5d0ea7c27ed9b157b21b8baa08a833-Steps.png">
...
Let's take the earlier [example](https://docs.clevertap.com/docs/constant-property#section-sending-a-summary). You can define the conditions for the target audience and then select the constant event property.
...
This will restrict the maximum number of values in constant event property per user to three. For users having lesser values, say two, they will get the message with only two show names. To know more about limits, see [Limit in Liquid Tags](doc:https://docs.clevertap.com/docs/liquid-tags#limit). By default, the limit is five (even if `limit` tag is not used)

You can also refer to a value in a particular order by referencing the index of that value in the array. For example, a user added to the cart but did not purchase red shoes, a yellow jacket, or a blue T-shirt. You can target users for the second item from the bottom that they added to the cart specifically. It can be achieved using the following script. The last item is index zero, the second last index is one, and so on.
...
* User counts for each value of the constant event property for notification sent, viewed, clicked, converted, and in control group.
...
* Mapping constant property with conversion event property allows you to view the conversion rate for specific property values.
* You can view metrics for any conversion event and track the conversion rate when any item is purchased or track the conversion rate for a specific item that is added to the cart.
