...
In this second scenario, the average time it takes a user to add an item to their cart (action 1) and then checkout (action 2) is five minutes. If the user has not completed the checkout process within five minutes of adding the item to their cart (inaction), then the likelihood that a user will checkout drops substantially. In this example, if you use the *Recommended Delay* feature which automatically recommends that you send a notification to the user if they have not completed the checkout process after five minutes.
...
<Image title="campaigns_push_type_Inaction.png" alt={1012} className="border" border={true} src="https://files.readme.io/62977ce-campaigns_push_type_Inaction.png" />
...
> 🚧 Note
>
> Event property personalization is available for **inaction campaigns** only if the inaction duration is within 24 hours.
