...
* For recurring campaigns, after approval and completion of a few runs, the creator can only update the message copy, and the campaign moves back to the Pending for approval state. For example, a non-profit organization runs a weekly donation reminder campaign.\
  The campaign has been approved and successfully sent out for a few weeks.n After a couple of successful runs, the campaign creator decides to update the message copy to make it more engaging. The campaign is automatically moved back to the *Pending for approval* state after the updates. The approver reviews the changes, and either approves the updated campaign to continue its recurring schedule or suggests further improvements.
