---
title: Notification Delivery Options (AI Generated)
...
> 🚧 Repeat Campaigns: First Day Delivery Behavior
>
> * CleverTap handles the first-day delivery differently based on the repeat frequency.
>   * **Daily or Weekly Repeat**:
>     * The campaign will begin dispatching messages on the same day it is launched, provided the current time is before the scheduled delivery time. 
>   * **Monthly Repeat**:
>     * The campaign does not dispatch messages on the day it is first launched. Delivery begins from the next scheduled monthly date. 
> * For example, if a monthly repeat campaign is created and published on January 01 at 10:30 AM, no messages will be sent on January 01. The first batch of notifications will be delivered on the next scheduled monthly cycle (February 01).
> * To send messages on the same day of publishing a monthly campaign, send a one-time campaign, or configure the start date from the next desired delivery date.
...
Select the *Cut-off* checkbox and specify the time to stop sending the notification when the number of target users is large. This will cause a significant delay in processing the user queue. This ensures that no notification is sent after the set time. 
...
**Time to Live** is the notification's persistence sent to an end user. It regulates the time the system monitors the user device status to deliver a push notification. The notification is delivered whenever the user is online during the set TTL period. Any user not coming online will not receive the push notification, and no notifications are attempted after the TTL period. For more information, refer to [Time to Live](doc:create-message-push#time-to-live).

# Message Frequency Caps

If multiple marketing campaigns run simultaneously for any channel, the same user may qualify to receive a message from more than one campaign in a given day, week, or month. Frequency caps allow you to limit the number of notifications a user will receive from all the campaigns running for that channel.
...
The error reporting section of the campaign report displays the number of users excluded from receiving the campaign due to the frequency cap setting.
...
Entry Limits allow you to control how many times a user can receive a campaign or journey message. Unlike **Message Frequency Caps**, which apply across all campaigns in a channel, Entry Limits work at the **campaign/journey level**, ensuring users don’t get overexposed to a specific message.  

This helps avoid fatigue, improves compliance, and ensures more meaningful engagement.  

## Set Up Entry Limits

You can configure Entry Limits from the **When** step of Campaign Builder and on each **Send Message** node in Journeys.  

1. Open the campaign or journey.  
2. Go to the **When** step (Delivery Preferences).  
3. Locate the **Message Entry Limits** card.  

<Image title="Message Entry Limits Card" alt={800} align="center" border={true} src="PLACEHOLDER_IMAGE_URL">
  Message Entry Limits Card in the When Step
</Image>

## Limit Options

You can choose one of the following mutually exclusive options:  

* **Send to all users**: The message is delivered every time a user qualifies.  
* **Send with a minimum interval**: Specify an interval in minutes, hours, or days. The same user will not receive the message again until that interval has passed.  
* **Send up to N times ever**: Define the maximum number of times a user can receive this message.  

<Image title="Limit Options" alt={900} align="center" border={true} src="PLACEHOLDER_IMAGE_URL">
  Entry Limit Options
</Image>

## Use Cases

* **E-commerce**: Send flash sale notifications up to 5 times ever to avoid oversaturation.  
* **Fintech**: Send a financial tip once after the first transaction.  
* **Food Delivery**: Send post-order feedback requests up to 10 times ever.  
* **Gaming**: Send major game update notifications up to 8 times ever.  
* **Subscriptions**: Send renewal reminders weekly, but only up to 6 times ever.  

## Notes and Behavior

* **Default value**: When selecting “Send up to N times ever,” the default is 1.  
* **Cloned campaigns**: Limits are copied when cloning a campaign, but each user starts with a fresh allowance.  
* **Supported channels**: Entry Limits are available for all Live and Recurring campaigns across Push, Email, SMS, App Inbox, Webhook, and more. One-time (single-send) campaigns do not support entry limits.  
* **Validation**: Inline validation ensures only valid values are entered. Invalid inputs trigger error messages.  

> 📘 Note
>
> In Delivery Analytics, a new metric **Blocked by Entry Limit** shows how many users were excluded due to limits.
