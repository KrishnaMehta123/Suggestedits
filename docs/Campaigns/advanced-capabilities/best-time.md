...
CleverTap identifies the best time based on a selected config. The  *Best Time* campaigns are sent by splitting a day into 12 buckets of two hours each in absolute clock time. The users are then assigned to one of these buckets based on their contextual Best Time. The user is then sent the campaign or journey in that two-hour window. If any user has not performed the selected event in the last 180 days, you can identify the fallback time for sending the campaign based on the user distribution graph.
