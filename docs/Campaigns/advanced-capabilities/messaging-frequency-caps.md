...
- **Segment Action Node with Sleep Time without Send at Specific Time**: When a sleep time is added after a node without enabling _Send at specific time_, CleverTap carries forward the throttle configuration from the previous engagement node. For example, if there is a sleep time of 1 day after a throttled engagement node, CleverTap ensures that the subsequent engagement is sent exactly one day after the previous engagement was delivered to each user. Allowing throttle configuration in the second engagement node, in this case, would introduce additional delays, as both nodes would apply throttling sequentially. This could lead to unintended message delivery gaps, extending the overall time taken for users to progress through the journey. (@Saby: Do we need a screenshot here?)
- **Segment Action Node with Sleep Time with Send at Specific Time**: When a sleep time is added with _Send at specific time_ enabled, CleverTap allows throttle configuration for the subsequent engagement node. Consider a scenario with a sleep time of 1 day set to trigger at 10:00 AM, and a throttle applied in the previous node. If a user received the first engagement at 10:00 AM on March 11, they would reach the next node at 10:00 AM on March 12. Users who received the first message later in the day on March 11 would then receive the next message at 10:00 AM on March 13, and so on. As the delivery is already controlled via a fixed time, additional throttling can be configured safely, helping to control volume without causing excessive delays. (@Saby: Do we need a screenshot here?)
