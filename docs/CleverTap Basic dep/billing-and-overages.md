...
> 
> Only the account administrators can access the _Organization_ section on the CleverTap dashboard.
...
The project admin can cancel the subscription by navigating to _Organization_ > _Billing_ > _My Plan_ > _Cancel Plan_. You are prompted to select the reason and enter _DELETE_ in the acknowledgement field. After the account is cancelled, all the projects and corresponding data under the organization are deleted permanently and cannot be retrieved.  
