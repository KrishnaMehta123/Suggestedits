---
title: Handling Unsubscribes_callback_WIP
excerpt: ''
deprecated: false
hidden: true
metadata:
...
2. Integrate the CleverTap JavaScript (JS) embed code, which is optimized for integration on both desktop and mobile web sites. Replace CLEVERTAP\_ACCOUNT\_ID with your actual Account ID in the JS. 
...
3. Go to the CleverTap Dashboard > *Settings* > *Amazon SES/Generic SMTP*. Now, only fill in the URL of the unsubscribe page with no parameters, such as [http://foobar.com/unsubscribe.html](http://foobar.com/unsubscribe.html).
