---
title: App Tracking Transparency (ATT)_WIP
excerpt: ''
deprecated: false
hidden: true
metadata:
...
---
# Overview
At the Apple Worldwide Developers Conference (iWWDC) 2020, Apple announced that all apps would require users to opt into tracking starting with iOS 14. This means that the *Identifier for Advertisers (IDFA)* can only be followed for users that explicitly opt-in. 
...
{
  "type": "warning",
  "body": "We recommend upgrading to the latest version of the CleverTap SDK for all customers using versions  3.8.2 and lower.",
  "title": "Important"
}
[/block]
## User Tracking
...
Please create a support ticket from the CleverTap dashboard if further information is needed. We would be happy to help you with any other queries.

# Revoke consent and delete data
Apple policy states that any third party with whom an app shares user data such as analytics tools, advertising networks, and third-party SDKs, as well as any parent, subsidiary, or other related entities that will have access to user data must provide the same or equal protection of user data as stated in the app's privacy policy and required by these Guidelines.

As an app owner, explain your (and CleverTap's) data retention/deletion policies and describe how a user can revoke consent or request deletion of the user's data.
You can easily remove app users who have revoked access to their data using our [Delete User Profile API](https://developer.clevertap.com/docs/delete-user-profile-api).
...
If you choose to implement ATT, we recommend passing the IDFA value as an event property with a *Custom Event* instead of setting it as a *Custom CleverTap ID*.