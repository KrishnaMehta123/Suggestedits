...
The [Subscribe API](https://developer.clevertap.com/docs/subscribe-api) allows you to subscribe or unsubscribe to a phone number. This is important so that you do not accidentally send an SMS to a phone number unless they have explicitly opted for it. 

## What is a _Profile not reachable_ error?

The _Profile not reachable_ error occurs when the qualified device is not reachable via the selected communication channel.
...
The following are the timeout and retry limits for SMS:
