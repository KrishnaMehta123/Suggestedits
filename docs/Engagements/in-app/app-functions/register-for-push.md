...
Android 13 (API level 33) introduced a runtime notification permission (`POST_NOTIFICATIONS`). Refer to [Android Notification Permission documentation](https://developer.android.com/develop/ui/views/notifications/notification-permission) for details.

* **Android 13 and above**: Push Primer triggers the system-level notification permission prompt when you set the button action to **Request Push Permissions** and the user clicks it.
* **Android 12 and below**: Notifications are allowed by default at install time. In this case, the Push Primer acts only as an educational layer to explain the benefits of notifications. The OS does not display the system permission dialog, and we do not explicitly request notification permission. For this reason, you should exclude such campaign deliveries to devices running Android 12 or lower.
