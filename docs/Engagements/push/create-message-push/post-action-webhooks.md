...
For API Push, add the `pa_webhook_name` key in the payload with the name of the webhook connection. For more information, refer to [Create Campaign API - Target User Events & Properties](https://staging.docs.dev.clevertap.net/docs/create-campaign-api-post-action-webhook#create-campaign-api---target-user-events--properties). 
...
Using a *post action* webhook, you can stream all the message-level data from a push campaign to your server. Following is the sample payload structure for push data: 
