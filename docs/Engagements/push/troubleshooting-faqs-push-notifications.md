...
Previously, filters in the _Who_ section were evaluated during campaign user qualification in the old segment builder. With the NextGen Segment Builder, this evaluation now happens during campaign dispatch. Here is how it works: 

Consider an example of a campaign that is triggered by user action and includes filters such as _region = India_ or _gender = Male_.
...
- **During the dispatching phase**, the user properties for the qualifying profiles are fetched, which will be used for evaluation.
  ...
  - Profiles with tokens are evaluated:
    - Those matching the filters (for example, _Region = India and Gender = Male_) receive the campaign notification.
