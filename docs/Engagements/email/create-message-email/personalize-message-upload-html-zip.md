...
When you upload an HTML template via ZIP file, CleverTap does not automatically detect personalization placeholders such as `{{ }}` or `@`. After uploading, you must manually add personalization fields using the Personalization toolbar during email campaign setup.
