...
In regions where template approval is required, RCS templates must be created and submitted in advance. Once approved:
...
Free-form templates allow you to create RCS message layouts directly in the RCS Message Editor without requiring provider approval. These templates provide complete control over text, media, placeholders, and button configuration, and are used in regions where template registration is not required.

To use a free-form template:

1. In the SMS Campaign Builder, select **RCS Message Provider** from the **Provider** dropdown.
2. Navigate to the **What** section.
3. Select a **Message Type** and click **Go to Editor**.
4. In the template selection screen, choose a template marked as free-form.
5. Click **Use Template** to open it in the RCS Message Editor.

In the RCS Message Template selection screen, the following free-form template types are available:

* **Text free-form template**
  A text-only format without media. Supports personalization and buttons.

* **Rich Card free-form template**
  A visual format that includes media, title, body text, and buttons.

* **Carousel free-form template**
  A scrollable set of rich cards, each with its own media, title, description, and buttons.

All fields in a free-form template are editable.

## Message Text

The Message Text field contains the main content that users receive.

Follow the steps below to configure Message Text:

1. In the RCS Message Editor, locate the **Message Text** field.
...
In approved templates, this field is locked. In free-form templates, the full message text is editable.

## Media (Rich Card and Carousel Templates)

To configure media:

1. Locate the **Media URL** field or media upload option.
2. Enter the URL of the media, or upload a file.
3. Add placeholders inside the Media URL if personalization is required.
4. For Carousel templates, repeat these steps for each card.

Refer to [Media URL](https://docs.clevertap.com/docs/rcs-message-editor#rich-card-template-1) for media configuration guidelines.

## Title (Rich Card and Carousel Templates)

To configure the Title:

1. Locate the **Title** field.
2. Enter the title text.
3. Add placeholders if personalization is needed.

## Body (Rich Card and Carousel Templates)

To configure the Body:

1. Locate the **Body** field.
2. Enter the descriptive text.
3. Add placeholders if personalization is required.

## Buttons

Free-form templates allow complete configuration of interactive buttons. You may add up to three buttons in Text and Rich Card templates, and up to two buttons per card in Carousel templates.
...
3. Select a button type:
   ...
   * **URL Redirect**: Opens a webpage. Refer to [URL Redirect Button](https://docs.clevertap.com/docs/rcs-message-editor#url-redirect-button).
   * **Dialer**: Opens the user’s phone dialer. Refer to [Dialer Button](https://docs.clevertap.com/docs/rcs-message-editor#dialer-button).
   * **Quick Reply**: Sends a predefined response. Refer to [Quick Reply Button](https://docs.clevertap.com/docs/rcs-message-editor#quick-reply-button).
4. Configure the required fields:

   * **Button Text**
   * URL, phone number, or reply text
   * **Postback** value

In approved templates, button settings are predefined. Free-form templates allow full edits.

## Fallback SMS

To configure Fallback SMS:

1. Turn on the **Fallback SMS** option.
2. Enter the **Template ID**, if required.
