...
A: Consider the inaction criteria is defined as users did A but not B within 2 minutes, with a node timeout set at 60 minutes. 
...
### Q. When multiple segments are connected to an action node, through which path does the user qualify?
...
### Q. Why is there a discrepancy between Via Sent count (Node Stats) and Sent count (Campaign Stats)?
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/6eb3233-image.png",
        null,
...
### Q. Why are the Waiting users - At Node numbers so high?
