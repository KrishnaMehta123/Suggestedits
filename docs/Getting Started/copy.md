...
The following image shows how to set up an event filter rule in an analytics or engagement platform. The rule builder excludes specific user events, such as a *ChargedAction* within the last 30 days, and applies \{@meenakshi, applies automatically or simply offers ?}additional filtering by narrowing down event properties. When users without decryption access query encrypted fields, the drop-down \{@meenakshi, it is not a dropdown it is a list}displays **This is an encrypted value** in the list to indicate restricted visibility of results.
