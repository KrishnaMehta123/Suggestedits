...
### Q. How do you configure the session invalidation timer for your dashboard?

A. A session invalidation timer helps you configure session timeout for your dashboard in case of inactivity. Only admins can configure the session invalidation timer. The default invalidation timer is set to 30 minutes.
...
3. Click the pencil icon ![](https://files.readme.io/f1ee3bd-edit_icon.png) to edit the timer. 
4. Select the relevant time from the dropdown, and click **Save**.

### Q. How do you log out from all active devices at once?

A. To log out from all active devices at once:
...
A. You can bulk upload user profiles using the CSV Upload feature. You can also use this feature to add or update information for existing user profiles.

To upload the CSV file to your CleverTap dashboard:

1. Login to your CleverTap account and click the *Settings* button.
2. Click  **CSV Uploads**.
3. Click **Import new profiles from CSV**.
...
### Q. How can I delete the event data from the dashboard without discarding events?

A. You can simply reset the DRP for each event. All the event data beyond the selected date range is discarded. 

For example, if you set the Data Retention Policy(DRP) of an event to 12 months, CleverTap stores the data for the past 12 months. All the data prior to the 12 months are deleted. You can optimize your container and avoid flooding it with too much data.  

To adjust the DRP for each event:

1. Navigate to *Settings* > *Schema* > *Events*.
2. Click the vertical ellipsis of the event you want to adjust/reduce the DRP.
3. Click **Set DRP**.

<Image title="New final Settings_DRP_Limit.png" alt={1163} align="center" className="border" border={true} src="https://files.readme.io/e8ea75f-New_final_Settings_DRP_Limit.png" />
...
<Image title="Set DRP.png" align="center" className="border" border={true} src="https://files.readme.io/faba57d-Set_DRP.png" />
