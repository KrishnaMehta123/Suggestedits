...
Access to PII is controlled through a secure vault and is used only during message delivery through approved, secure CPaaS integrations. This ensures that PII is accessed only at send time, for delivery purposes, and only through compliant pipelines.
...
2. Vault SDK returns tokens.
3. Tokens are sent to CleverTap.
4. During message execution, CleverTap’s message dispatch server requests PII from the token vault using the [Get Value](https://staging.docs.dev.clevertap.net/docs/get-value-api) API.
...
The On-Prem server resolves tokens via the token vault and delivers the final payload to the CPaaS provider.
...
| Vault SDK            | Ensures apps exchange only tokens with CleverTap.           |
...
The following steps outline the setup required to enable secure Tokenization with CleverTap, from preparing your token vault to integrating the SDK, configuring message dispatch, and validating the full workflow before production rollout:

1. [Prepare the Token Vault](doc:pii-tokenization#prepare-the-token-vault)
2. [Integrate the Vault SDK](doc:pii-tokenization#integrate-the-vault-sdk)
3. [Configure Message Dispatch](doc:pii-tokenization#integrate-the-vault-sdk)
4. [Test the Integration](doc:pii-tokenization#test-the-integration)
5. [Rollout and Monitor](doc:pii-tokenization#rollout-and-monitor)
...
### Integrate the Vault SDK

Embed tokenization directly into your app flows so that raw PII never reaches CleverTap.

{@Sagar, Do I have to remove this section or link to the Github steps, as discussed?}

- Install the CleverTap Vault SDK in your mobile and web apps.
- Update your app flows so that PII is exchanged with the Vault and replaced by tokens before data is sent to CleverTap.
- Confirm that only tokens and non-PII values appear in CleverTap dashboard to ensure PII never enters CleverTap systems.

### Configure Message Dispatch

Enable the dispatch layer to securely resolve tokens into PII during message delivery.

- **Off-Prem Dispatch**

  - Allow CleverTap’s dispatch server to securely call the [Get Value API](https://staging.docs.dev.clevertap.net/docs/get-value-api) during message delivery.
...
[block:parameters]
{
  "data": {
    ...
    "1-0": "CleverTap",
    "1-1": "Provide the Vault SDK for token exchange.<br>- Provide and support the On-Prem Dispatcher (if applicable).<br>- Ensure CleverTap processes and stores only tokens and non-PII data."
  },
  "cols": 2,
  "rows": 2,
  "align": [
