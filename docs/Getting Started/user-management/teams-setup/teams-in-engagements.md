...
The [Teams](doc:teams-feature-setup-guide) feature in CleverTap introduces scoped access for both engagements (campaigns and journeys), ensuring that users only view, create, manage, and publish items that belong to their assigned teams. This enforces cleaner ownership and stricter governance and prevents cross-team content visibility.
...
Teams work alongside Roles to define both **what users can do** (permissions) and **what they can access** (team-scoped entities). For more information, refer to [User Roles and Access](doc:teams-feature-setup-guide#user-roles-and-access).
...
<Image alt="Assign Journey to a Team" align="center" border={true} src="https://files.readme.io/7d45c57d3e5eb4eaa2c04629a17645f089b20dd48872dc8c2644ea6fc1bf0d4f-Team_Selection_for_Journeys.png">
...
When a team is assigned to a user, the events accessible in campaigns are scoped to that team’s configuration. For more information on setting up Events in Teams, refer to [Event Visibility in Teams](https://staging.docs.user.clevertap.net/docs/teams-setup#event-visibility-in-teams).
...
For setup details, refer to [Teams: Setup](https://staging.docs.user.clevertap.net/docs/teams-feature-setup-guide) and for segment-specific rules, refer to [Teams in Segmentation](https://staging.docs.user.clevertap.net/docs/teams-in-segmentation). 
