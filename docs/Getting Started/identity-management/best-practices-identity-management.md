...
- If your app allows you to change email addresses, enable _Do not merge identifiers on the same device_ on your CleverTap dashboard. For the steps, refer to [How Do You Want CleverTap to Handle Identified Profiles?](https://docs.clevertap.com/docs/troubleshooting#how-do-you-want-clevertap-to-handle-identified-profiles).
- Do not set an identity if it is not present. 

> 👍 Example
> 
> Setting an identity as _None_ to multiple users will group all events under that _None_ identity together.
> 
> Any user with the _None_ identity is assumed to be the same user. You can always set the identity later and CleverTap has in-built logic that will merge the anonymous events to the identified user. 
> 
