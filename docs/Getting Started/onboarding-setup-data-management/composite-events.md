...
excerpt: Understand what are Composite events and their properties
deprecated: false
hidden: false
metadata:
  title: ''
...
Composite events provide you with the ability to combine multiple events. A composite event can be used as a single event on the dashboard, which can be analyzed in reports.  
...
3. Name your event and select the events that you want to group. In our example, we name the event as *Travel Bookings*. 
4. Select the events and filter by properties, if required. In our example, we filter the *Hotel Booked* event by the event property *Rating*, which is three stars, and the flight booked event by the event property *Class* (*Economy*).  
...
Now that you have created a composite event, you can create a composite property using the properties of the underlying events. You can create a composite event property by grouping two or more underlying event properties together. After they are grouped, they behave as a single event property. 

For example, we create an event property called *Booking* revenue that groups the underlying properties such as *Flight* booked amount (from the flight booked event) and *Hotel* booked amount (from the hotel booked event). When you analyze the *Booking* revenue property, it will function as a single event property. 
...
You can now use this composite property instead of two separate underlying properties. For example, you can view a trend of the *Booking Revenue* property in the [Trends](https://docs.clevertap.com/v1.5/docs/trends#section-view-a-trend) report. 
...
You can add any properties that belong to any underlying events as individual property to the composite event. 

The composite event behaves as a single event, and this individual property acts as its underlying property. Therefore, you can stop referencing the underlying event to analyze a specific property. 

For example, if you want to analyze an event called *Flight Class* from the flight booked event, you can add this event to the main composite event *Travel Bookings*, then use it as if it is an underlying property of this event. You do not need to use the Flight Booked event because the new composite property offers a better view of your events. 
...
You have now grouped the underlying properties. 
